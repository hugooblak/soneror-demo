...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'soneror-leads' ),
					'singular_name' => __( 'Lead', 'soneror-leads' ),
					'edit_item'     => __( 'Förfrågan', 'soneror-leads' ),
					'search_items'  => __( 'Search leads', 'soneror-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'soneror-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'soneror-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'soneror-leads' ),
			'npl_phone'    => __( 'Phone', 'soneror-leads' ),
			'npl_service'  => __( 'Needs', 'soneror-leads' ),
			'npl_timeline' => __( 'When', 'soneror-leads' ),
			'npl_zip'      => __( 'ZIP', 'soneror-leads' ),
			'npl_source'   => __( 'Source', 'soneror-leads' ),
			'npl_status'   => __( 'Status', 'soneror-leads' ),
			'date'         => __( 'Received', 'soneror-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'soneror-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'soneror-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'soneror-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'soneror-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'soneror-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'soneror-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'soneror-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'soneror-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'soneror-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'soneror-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'soneror-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'soneror-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
