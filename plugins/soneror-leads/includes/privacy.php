...
add_filter(
	...
	function ( $exporters ) {
		$exporters['soneror-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'soneror-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'soneror-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'soneror-leads',
						'group_label' => __( 'Quote requests', 'soneror-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['soneror-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'soneror-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
