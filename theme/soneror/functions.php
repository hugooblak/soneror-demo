<?php
/**
 * Söne Rör AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Söne Rör AB Leads" plugin, so leads survive a theme change.
 *
 * @package Söne Rör AB
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'soneror', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'soneror' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'soneror' ) ) );

		register_block_pattern_category( 'soneror-sections', array( 'label' => __( 'Söne Rör AB sections', 'soneror' ) ) );
		register_block_pattern_category( 'soneror-pages', array( 'label' => __( 'Söne Rör AB page layouts', 'soneror' ) ) );
	}
);
